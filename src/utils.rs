...
pub fn find_best_match<'a>(input: &str, candidates: impl Iterator<Item = &'a String>) -> Option<String> {
...
pub fn ordinal(n: usize) -> String {
    let s = n.to_string();
    if s.ends_with('1') && !s.ends_with("11") { format!("{}st", n) }
    else if s.ends_with('2') && !s.ends_with("12") { format!("{}nd", n) }
    else if s.ends_with('3') && !s.ends_with("13") { format!("{}rd", n) }
    else { format!("{}th", n) }
}